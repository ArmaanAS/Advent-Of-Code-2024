...
const mapScores = map.map((row) =>
  row.map(() => [
    { fromStart: Infinity, toEnd: Infinity },
    { fromStart: Infinity, toEnd: Infinity },
    { fromStart: Infinity, toEnd: Infinity },
    { fromStart: Infinity, toEnd: Infinity },
  ])
...
function getDir(x: number, y: number) {
  ...
  if (y < 0) return 2;
  if (y > 0) return 3;
  throw new Error("0,0 not allowed");
}
...
function dfs(
  ...
  const dir = getDir(x - px, y - py);
  const mapScore = mapScores[y][x][dir];
...
for (let y = 0; y < height; y++) {
  ...
  for (let x = 0; x < height; x++) {
    const scores = mapScores[y][x];
    const score = Math.min(...scores.map((s) => s.fromStart + s.toEnd));
    if (score === bestScore) {
      row += "O";
      bestTiles += 1;
    } else {
